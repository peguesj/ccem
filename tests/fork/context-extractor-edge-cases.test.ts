...
describe('Context Extraction Engine - Edge Cases', () => {
  describe('extractByFile - Advanced Cases', () => {
    it('should handle file without dependencies', () => {
      const conv: Conversation = {
        messages: [
          ...
          { role: 'assistant', content: 'Creating standalone.ts', timestamp: '2025-10-17T00:01:00Z' }
        ],
        files: ['standalone.ts'],
    ...
    it('should match files with path separators', () => {
      const conv: Conversation = {
        messages: [
          { role: 'user', content: 'Create src/utils/helper.ts', timestamp: '2025-10-17T00:00:00Z' },
          { role: 'assistant', content: 'Creating src/utils/helper.ts', timestamp: '2025-10-17T00:01:00Z' }
        ],
    ...
    it('should find related test files with various patterns', () => {
      const conv: Conversation = {
        messages: [
          { role: 'user', content: 'Create component.tsx and test', timestamp: '2025-10-17T00:00:00Z' },
      ...
      expect(context.dependencies!.some(d => d.includes('test') || d.includes('spec'))).toBe(true);
    });
    ...
    it('should detect config node type', () => {
      ...
      graph.nodes.forEach(node => {
        if (
          ...
          node.id.endsWith('.yml') ||
          node.id.endsWith('.yaml')
        ) {
  ...
  describe('buildDependencyGraph - Source File Detection', () => {
    it('should find source file for test in tests/ directory', () => {
      ...
      const testToSourceEdge = graph.edges.find(e =>
        e.from === 'tests/helper.test.ts' && e.to === 'src/helper.ts'
      );
    ...
    it('should find source file for test in __tests__/ directory', () => {
      ...
      const testToSourceEdge = graph.edges.find(e =>
        e.from.includes('__tests__') && e.type === 'tests'
      );
